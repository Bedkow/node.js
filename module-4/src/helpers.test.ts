...
describe("Helper functions work as intended", ()=>{
    ...
    it("validates input of '2023', 'DE'", () => {
        ...
        expect(()=>{validateInput({year: 2023, country: "XD"})}).toThrow(new Error(`Country provided is not supported, received: XD`));
        expect(()=>{validateInput({year: 5023, country: "DE"})}).toThrow(new Error(`Year provided not the current, received: 5023`))
    });
    ...
    it("shortens public holiday entry", () => {
        expect(shortenPublicHoliday({
          ...
          })).toEqual({name: "New Year's Day", localName: "Neujahr", date: "2023-01-01"})
    })
});